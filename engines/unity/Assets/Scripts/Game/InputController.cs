...
public class InputController : MonoBehaviour
{
    ...
    public readonly List<Note> TouchableNormalNotes = new List<Note>(); // Click, CDrag head, Flick
    ...
    /// <summary>
    /// Yield Click/CDrag-head/Hold candidates in beat order: cluster by effectiveNoteTime span ≤
    /// <see cref="NoteClusterGapSeconds"/> (no chain expansion), process earlier
    /// clusters first; within a cluster prefer non-Hold, then closer rendered center X,
    /// then time, then id. Soft fallthrough: caller continues when Accept fails.
