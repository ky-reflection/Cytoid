...
| 2026-08 | Hold release is judged in `OnGameUpdate`; 0-tick completes on `Time >= end` while held | `GameTouchInput` is -100, so release ran before `Time` / `HeldDuration` ticked. Worse for 0-tick (end == start): `Time >= end` cleared fingers but `Time > start` skipped `Clear`, leaving `HoldingNotes` bound and `IsHolding` false — swipe-on-and-stay never rebound, tap (Down + same-frame tick past start) worked. Completion is `Time >= end` while held; `UpdateFinger` does not judge. |
