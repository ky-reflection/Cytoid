...
| 2026-08 | Hold release is judged in `OnGameUpdate`, not `UpdateFinger` | `GameTouchInput` is execution order -100, so FingerUp / slide-off ran before `Game` advanced `Time` and ticked `HeldDuration`. Immediate `Clear` on release used a one-frame-stale duration — swipe-through on an ultra-short hold Missed while a tap that stayed for the hold tick completed. `UpdateFinger` only binds/releases; `HoldNote.OnGameUpdate` syncs duration then grades. |
